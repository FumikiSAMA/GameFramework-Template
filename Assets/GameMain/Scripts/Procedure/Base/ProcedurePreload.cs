...
namespace Fumiki
{
    public class ProcedurePreload : ProcedureBase
    {
        ...
        private void PreloadResources()
        {
            ...
            // Preload data tables
            foreach (string dataTableName in AssetNames.DataTableNames)
            {
            ...
            // Preload fonts
            foreach (string fontName in AssetNames.FontNames)
            {
                LoadFont(fontName);
            }
        }
        ...
        private void LoadFont(string fontName)
        {
            ...
            GameEntry.Resource.LoadAsset(AssetUtility.GetFontAsset(GameEntry.Setting.GetString(Constant.Setting.Language), fontName), Constant.AssetPriority.FontAsset, new LoadAssetCallbacks(
                (assetName, asset, duration, userData) =>
                {
                    _loadedFlag[Utility.Text.Format("Font.{0}", fontName)] = true;
                    GameEntry.StaticResource.FontAssets.Add(fontName, (TMP_FontAsset) asset);
